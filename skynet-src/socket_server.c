...
struct socket {
    ...
    int64_t wb_size;            // 写入缓冲大小
    int fd;                     // 文件描述符
    int id;                     // 可能是服务端弄的ID
    uint8_t protocol;           // 协议
    uint8_t type;               // SOCKET事件通知类型
    uint16_t udpconnecting;     // 不知道
    int64_t warn_size;          // 警告大小
    union {
        int size;
        uint8_t udp_address[UDP_ADDRESS_SIZE];
    } p;
...
struct socket_server {
    int recvctrl_fd;        // 管道读取控制fd
    int sendctrl_fd;		// 管道发送控制fd
    int checkctrl;			// 是否有控制命令，如果有则是1,如果没有则是0，-1表示退出
    poll_fd event_fd;		// 对应的epoll的对象
    int alloc_id;			// 分配ID（从0开始）
    int event_n;			//
    int event_index;
    struct socket_object_interface soi;
    struct event ev[MAX_EVENT];               // epoll的所有事件
    struct socket slot[MAX_SOCKET];           // 预先分配的socket object槽
...
// 请求设置SSL属性
// add by joole --begin
struct request_setssl {
    int id;             // 只对服务端设置的SSL
    int role;           // 表明是否是server
};
...
/**
 * 发送对象检测,根据缓冲政府
 * @param : socket_server *ss : socket上下文
 * @param : 发送对象结构
 * @param : 发送的缓冲
 * @param : 发送的缓冲大小
 * return : 
    ...
    for (i=0;i<MAX_SOCKET;i++) {
        ...
        if (s->type == SOCKET_TYPE_INVALID) {
            if (ATOM_CAS(&s->type, SOCKET_TYPE_INVALID, SOCKET_TYPE_RESERVE)) {
                s->id = id;
                // socket_serverf_udp_connect may inc s->udpconncting directly (from other thread, before new_fd),
...
init_server_ctx() 
{
    ...
    if (ctx == NULL) {
        skynet_error(NULL, "can't initialize SSL Library");
        return NULL;
...
static void 
setssl_socket(struct socket_server *ss, struct request_setssl *request)
{
    int id = request->id;
    struct socket *s = &ss->slot[HASH_ID(id)];
    if (s->type == SOCKET_TYPE_INVALID || s->id != id) {
...
/// \brief : get socket cmd and handle it
/// \param : socket_server *ss 全局的socket的管家
/// \param : socket_message *result 准备获取的消息
static int
ctrl_cmd(struct socket_server *ss, struct socket_message *result) {
    ...
    case 'U':
        ...
        return -1;
    case 'E':
        setssl_socket(ss, (struct request_setssl *)buffer);
        return -1;
    default:
...
// 就是用来对接受的fd产生新的SOCKET的操作，非常好的切入点
...
/// \brief 服务端循环历程
/// \param : socket_server *ss : 服务端的socket上下问
/// \param : socket_message *result : socket消息（用于整个socket系统的相互通知）
/// \param : int* more : 是否还有个呢更多
/// \return : int ： 返回处理socket的类型，以便后继续处理
int
...
// add by joole --begin
void
socket_server_setssl(struct socket_server *ss, int id) {
    struct request_package request;
    request.u.setssl.id = id;
    request.u.setssl.role = 1;
    send_request(ss, &request, 'E', sizeof(request.u.setssl));
}
